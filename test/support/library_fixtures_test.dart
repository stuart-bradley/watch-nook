...
void main() {
  ...
  test('seedShow derives progress columns rather than accepting them', () async {
  ...
  test('seedShow dedupes like the app: same title twice is one row', () async {
    await seedShow(db, tmdbId: 95396);
    await seedShow(db, tmdbId: 95396);

    expect(await db.libraryDao.getAll(), hasLength(1));
  });

  test('seedShow marks idempotently — a repeated coordinate is one row', () async {
  ...
  test('seedShow does not invent an episode runtime from the show average', () async {
    ...
    expect(
      events.single.runtimeMinutes,
      isNull,
      reason: 'the show average is not the episode runtime; stats must not '
          'read a fabricated snapshot as though it were measured',
    );
  });

  test('seedRawItem can fabricate the broken state the repair tests need', () async {
